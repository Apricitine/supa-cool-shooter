...
(function(window) {
  ...
  var Class = function(methods) {
    ...
    for (var key in methods) if (methods.hasOwnProperty(key)) ret.prototype[key] = methods[key];
    return ret;
  };
  ...
  var Fx = new Class({
    ...
    tweenStart: function(key, time) {
      ...
      var values = [];
      for (var i = 0,
      tween; tween = this.tweens[key].tweens[i]; i++) values.push(tween[0]);
      this.fire(key, values, 0);
    },
    tweenFinished: function(tween, key) {
      var values = [];
      for (var i = 0,
      t; t = tween.tweens[i]; i++) values.push(t[1]);
    ...
    fire: function(key, values, delta) {
      for (var i = 0,
      listener; listener = this.listeners[i]; i++) listener.set.call(listener, key, values, delta);
    }
  });
  var Tween = {
    Linear: function(x) {
  ...
  function each(arr, func, bindObject) {
    ...
    for (var key in arr) if (arr.hasOwnProperty(key)) func.call(bindObject || window, arr[key], key);
    return arr;
  }
  ...
  function setStyles(element, props) {
    for (var key in props) if (props.hasOwnProperty(key)) {
      var val = props[key];
      if (typeof val === "number" && key !== "opacity" && key !== "zIndex") val = val + 'px';
  ...
  var KickAss = new Class({
    ...
    loop: function() {
      ...
      this.updateWindowInfo();
      for (var i = 0,
      player; player = this.players[i]; i++) {
    ...
    isKickAssElement: function(el) {
      for (var i = 0,
      element; element = this.elements[i]; i++) {
  ...
  var MenuManager = new Class({
    ...
    create: function() {
      ...
      addEvent(this.submitScoreLink, 'click', bind(this,
      function(e) {
        stopEvent(e);
        this.navigateTo('highscores');
      }));
      addEvent(this.menuLink, 'click', bind(this,
      function(e) {
        stopEvent(e);
        this.toggleMenu();
        this.navigateTo('main', true);
      }));
      addEvent(this.switchShipLink, 'click', bind(this,
      function(e) {
        stopEvent(e);
        this.navigateTo('ships');
      }));
      addEvent(this.bombLink, 'click', bind(this,
      function(e) {
        stopEvent(e);
        this.game.fireBomb();
      }));
    ...
    includeCSS: function(file) {
      ...
      link.rel = 'stylesheet';
      link.type = 'text/css';
      link.href = file; (document.head || document.body).appendChild(link);
    },
  ...
  var UIManager = new Class({
    ...
    getLowestBubbleY: function() {
      var top = 100;
      for (var id in this.messages) if (this.messages.hasOwnProperty(id)) top = Math.max(this.messages[id].kickassto + this.messages[id].offsetHeight + 10, top);
      return top;
    }
  });
  var AudioManager = new Class({
    ...
    buildAudioElement: function() {
      ...
      for (var i = 0,
      format; format = this.formats[i]; i++) {
    ...
    audioEnded: function(target) {
      for (var i = 0,
      channel; channel = this.channels[i]; i++) {
  ...
  var Player = new Class({
    ...
    setShip: function(ship) {
      ...
      this.verts = [];
      for (var i = 0,
      vert; vert = this.ship.points[i]; i++) this.verts.push(new Vector(vert[0], vert[1]));
    ...
    setCannons: function(cannonClass) {
      var newCannons = [];
      for (var i = 0,
      cannon; cannon = this.cannons[i]; i++) {
    ...
    update: function(tdelta) {
      if (this.isBroken) {
        if (!this.lineOffsets.length) {
          for (var i = 0; i < (this.verts.length - 1); i++) this.lineOffsets[i] = {
            pos: new Vector(0, 0),
      ...
      if (this.currentRotation) this.dir.setAngle(this.dir.angle() + this.currentRotation * tdelta);
      var frictionedAcc = this.acc.mulNew(tdelta).sub(this.vel.mulNew(tdelta * this.friction));
      this.vel.add(frictionedAcc);
      if (this.vel.len() > this.terminalVelocity) this.vel.setLength(this.terminalVelocity);
      ...
      for (var i = 0,
      thruster; thruster = this.thrusters[i]; i++) {
      ...
      if (!this.lastPos.is(this.pos) || this.currentRotation || this.forceRedraw) {
        ...
        if (showFlames) {
          for (var i = 0,
          thruster; thruster = this.thrusters[i]; i++) thruster.drawTo(this.sheet);
        }
    ...
    inRect: function(rect) {
      var ret = false;
      for (var i = 0,
      vert; vert = this.verts[i]; i++) {
        if (rect.hasPoint(new Vector(vert.x + this.pos.x, vert.y + this.pos.y))) ret = true;
      }
      return ret;
    },
    hit: function(by) {
    ...
    getSizeFromVertsAndObjects: function() {
      ...
      for (var i = 0,
      vert; vert = this.verts[i]; i++) largestDistance = Math.max(largestDistance, (new Vector(vert)).len());
      for (var i = 0,
      obj; obj = this.thrusters[i]; i++) {
    ...
    shootPressed: function() {
      for (var i = 0,
      cannon; cannon = this.cannons[i]; i++) cannon.shootPressed();
    },
    shootReleased: function() {
      for (var i = 0,
      cannon; cannon = this.cannons[i]; i++) cannon.shootReleased();
    },
  ...
  var Thruster = new Class({
    ...
    generateFlames: function() {
      ...
      for (var x = 0; x < redWidth; x += redIncrease) this.flames.r.push(vec(x - halfRed + px, this.flameY * random(this.size.height * 0.7, this.size.height) + py));
      this.flames.r.push(vec(halfRed + px, py));
      for (var x = 0; x < yellowWidth; x += yellowIncrease) this.flames.y.push(vec(x - halfYellow + px, this.flameY * random(this.size.height * 0.4, this.size.height * 0.7) + py));
      this.flames.y.push(vec(halfYellow + px, py));
      this.lastFrameUpdate = now();
      var pos = vec(px, py);
      for (var i = 0,
      p; p = this.flames.r[i]; i++) p.sub(pos).rotate(this.angle).add(pos);
      for (var i = 0,
      p; p = this.flames.y[i]; i++) p.sub(pos).rotate(this.angle).add(pos);
    }
  });
  ...
  var BulletManager = new Class({
    ...
    endBlink: function() {
      for (var i = 0,
      el; el = this.enemyIndex[i]; i++) el.style.outline = el.KICKASSOLDBORDER;
      this.lastBlink = 0;
      this.blinkActive = false;
    },
    ...
    isDestroyable: function(element, ignoreSize) {
      ...
      for (var i = 0,
      child; child = element.childNodes[i]; i++) {
    ...
    shouldIgnoreElement: function(element, ignoreSize) {
      ...
      if (ELEMENTSTHATARENOTTOBEINCLUDED.indexOf(element.tagName) !== -1) return true;
      if (element.style.visibility == 'hidden' || element.style.display == 'none') return true;
      if (typeof element.className == "string" && element.className.indexOf('KICKASSELEMENT') != -1) return true;
      if (!ignoreSize) {
        if (element.offsetWidth < ELEMENTSIZETHRESHOLD || element.offsetHeight < ELEMENTSIZETHRESHOLD) return true;
      }
      var rect;
      if (element.offsetLeft < 0 || element.offsetTop < 0) {
        rect = getRect(element);
        if (rect.left + rect.width < 0 || rect.top + rect.height < 0) return true;
      }
    ...
    destroy: function() {
      for (var key in this.bullets) if (this.bullets.hasOwnProperty(key)) for (var i = 0,
      bullet; bullet = this.bullets[key][i]; i++) bullet.destroy();
      this.bullets = {};
    }
  });
  ...
  var ExplosionManager = new Class({
    ...
    update: function(tdelta) {
      var time = now();
      for (var i = 0,
      explosion; explosion = this.explosions[i]; i++) {
    ...
    destroy: function() {
      for (var i = 0,
      explosion; explosion = this.explosions[i]; i++) explosion.destroy();
      this.explosions = [];
    }
  });
  ...
  var BallCannon = new Class({
    ...
    removeOld: function() {
      var time = now();
      for (var i = 0,
      bullet; bullet = this.bullets[i]; i++) {
  ...
  var ParticleExplosion = new Class({
    ...
    update: function(tdelta) {
      for (var i = 0,
      particle; particle = this.particles[i]; i++) particle.pos.add(particle.vel.mulNew(tdelta).mul(random(0.5, 1.0)).setAngle(particle.dir.angle()));
      this.sheet.clear();
      this.sheet.drawExplosion(this.particles);
    },
    generateParticles: function() {
      for (var i = 0,
      j = !GameGlobals.hasCanvas ? 10 : 40; i < j; i++) {
        this.particles.push({
  ...
  var SplitExplosion = new Class({
    ...
    createClones: function() {
      ...
      return each([leftContainer, rightContainer],
      function(el) {
  ...
  var Sheet = new Class({
    ...
    drawBrokenPlayer: function(verts, lineOffsets) {
      ...
      this.drawer.setLineWidth(1.5);
      for (var i = 1,
      vert, lastVert = verts[0]; vert = verts[i]; i++, lastVert = vert) {
    ...
    drawExplosion: function(particles) {
      for (var i = 0,
      particle; particle = particles[i]; i++) {
